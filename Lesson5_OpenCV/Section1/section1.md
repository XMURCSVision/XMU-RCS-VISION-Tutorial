...
| **功能** | **API 名称**              | **调用示例**                                       | **参数列表**                        | **默认参数说明**                           |
| ------ | ----------------------- | ---------------------------------------------- | ------------------------------- | ------------------------------------ |
| 读取图像   | `cv::imread`            | `cv::imread("../image1.jpg", cv::IMREAD_COLOR)` | `imread(<src>, <opt>)`          | `<opt> = cv::IMREAD_COLOR` 彩色读取      |
| 显示图像   | `cv::imshow`            | `cv::imshow("window", src)`                    | `imshow(<name>, <src>)`         | 无默认参数                                |
| 等待键盘事件 | `cv::waitKey`           | `cv::waitKey(0)`                               | `waitKey(<opt>)`                | `<opt> = 0` 表示无限等待                   |
| 销毁所有窗口 | `cv::destroyAllWindows` | `cv::destroyAllWindows()`                      | `destroyAllWindows()`           | 无默认参数                                |
| 销毁特定窗口 | `cv::destroyWindow`     | `cv::destroyWindow("window")`                  | `destroyWindow(<name>)`         | 无默认参数                                |
| 创建窗口   | `cv::namedWindow`       | `cv::namedWindow("window", cv::WINDOW_NORMAL)` | `namedWindow(<name>, <opt>)`    | `<opt> = cv::WINDOW_AUTOSIZE` 自动适应大小 |
| 保存图像   | `cv::imwrite`           | `cv::imwrite("../output.jpg", src)`               | `imwrite(<name>, <src>, <opt>)` | `<opt> = std::vector<int>()` 可省略     |
...
注意在显示帧时，请给`cv.waitKey()`函数传递适当的时间参数。如果它太小，视频将非常快，如果它太高，视频将会很慢。在正常情况下，25毫秒就可以了。
```C++
// 1. 打开一个mp4
