...
: Node("armor_detector", options)  //父类Node初始化
{
  RCLCPP_INFO(this->get_logger(), "Starting DetectorNode!");

  // Detector
  detector_ = initDetector();  //159行
  ...
  /*armor_maker_定义在.hpp第56行  
    visualization_msgs::msg::Marker armor_marker_; */
  //以下都是visualization_msgs::msg::Marker的字段
  ...
  if (pnp_solver_ != nullptr) {
    ...
    for (const auto & armor : armors) {
      ...
      if (success) {
        ...
        // rvec to 3x3 rotation matrix 将旋转向量rvec转为旋转矩阵matrix
        cv::Mat rotation_matrix;
        cv::Rodrigues(rvec, rotation_matrix);

        // rotation matrix to quaternion   旋转矩阵转为四元数
        tf2::Matrix3x3 tf2_rotation_matrix(  //新开一个矩阵，将原旋转矩阵的值提取到tf2旋转矩阵
        ...
        /*tf2是ros2中处理坐标变换的库，能进行四元数、旋转矩阵、欧拉角之间的转化
              tf2::Quaternion四元数类型*/
        tf2_rotation_matrix.getRotation(tf2_q);
        //将矩阵的值提取出来，转换为四元数
        armor_msg.pose.orientation = tf2::toMsg(tf2_q);
        //第139行是将tf2::Quaternion四元数类型转为ros2消息中的四元数格式geometry_msgs::Quaternion*/
  ...
  const sensor_msgs::msg::Image::ConstSharedPtr & img_msg)
{
  ...
  auto armors = detector_->detect(img);  // armors 是std::vector<Armor>
