...
namespace rm_auto_aim
{
  Detector::Detector(
      ...
      : binary_thres(bin_thres), detect_color(color), l(l), a(a) // 传参，成员变量初始化
  {
  }
  ...
  {
    // input是原rgb图像
    binary_img = preprocessImage(input);     // 43行
    lights_ = findLights(input, binary_img); // 69行
    armors_ = matchLights(lights_);          // 118行
  ...
  {
    cv::Mat gray_img;
    cv::cvtColor(rgb_img, gray_img, cv::COLOR_RGB2GRAY); // RGB to GRAY
    /*
    void cv::cvtColor(
     InputArray src,  // 源图像
     OutputArray dst, // 目标图像
     int code,   // 颜色空间转换代码
     int dstCn = -1 )  // 目标图像的通道数，默认为-1，表示根据转换代码自动确定
    */
  ...
  std::vector<Light> Detector::findLights(const cv::Mat &rbg_img, const cv::Mat &binary_img)
  {
    ...
    void cv::findContours(
      ...
      OutputArrayOfArrays contours, // 输出参数，存储检测到的轮廓，每个轮廓是一个点的向量
      OutputArray hierarchy, // 输出参数，存储轮廓的层次结构信息，如果不需要可以是空矩阵
      int mode, // 轮廓检索模式
      int method )  // 轮廓逼近方法
    */
